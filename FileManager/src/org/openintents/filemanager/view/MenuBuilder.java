...
public class MenuBuilder implements Menu {
	...
	List<MenuItemImpl> findItemsWithShortcutForKey(int keyCode, KeyEvent event) {
		...
		for (int i = 0; i < N; i++) {
			MenuItemImpl item = mItems.get(i);
			if (item.hasSubMenu()) {
				List<MenuItemImpl> subMenuItems = ((MenuBuilder) item
						.getSubMenu()).findItemsWithShortcutForKey(keyCode,
						event);
	 ...
	 * 
	 * @param cleared
	 *            Whether the items were cleared or just changed.
	 */
	private void onItemsChanged(boolean cleared) {
		if (!mPreventDispatchingItemsChanged) {
			if (mIsVisibleItemsStale == false)
				mIsVisibleItemsStale = true;
		}
	}
