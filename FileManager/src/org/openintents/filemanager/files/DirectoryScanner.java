...
public class DirectoryScanner extends Thread {
	...
	public static final int MESSAGE_SHOW_DIRECTORY_CONTENTS = 500; // List of
																	// contents
																	// is ready,
																	// obj =
																	// DirectoryContents
	public static final int MESSAGE_SET_PROGRESS = 501; // Set progress bar,
														// arg1 = current value,
														// arg2 = max value
	...
	private File[] files;
	/**
	 * We keep all these three instead of one, so that sorting is done
	 * separately on each.
	 */
	...
	private void init() {
		...
		sdIcon = context.getResources().getDrawable(
	...
	public void run() {
		...
		if (files != null) {
			for (File currentFile : files) {
				...
				if (currentFile.getName().equalsIgnoreCase(
						FileUtils.NOMEDIA_FILE_NAME))
					noMedia = true;

				// If the user doesn't want to display hidden files and the file
				// is hidden, ignore this file.
				...
				if (currentFile.isDirectory()) {
					...
					if (currentFile.getAbsolutePath().equals(mSdCardPath)) {
						listSdCard.add(new FileHolder(currentFile, mMimeTypes
								.getMimeType(currentFile.getName()), sdIcon,
								context));
					}
					// It's a normal directory.
					else {
						if (!mWriteableOnly || currentFile.canWrite())
							listDir.add(new FileHolder(currentFile, mMimeTypes
									.getMimeType(currentFile.getName()),
									folderIcon, context));
					}
					...
					boolean ext_allow = filetype
							.equalsIgnoreCase(mFilterFiletype)
							|| mFilterFiletype == "";
					...
					if (!mDirectoriesOnly && (ext_allow || mime_allow)) {
						// Take advantage of the already parsed mimeType to set
						// a specific icon.
						listFile.add(new FileHolder(currentFile, mimetype,
								genericFileIcon, context));
					}
				}
			}
		}
		...
		if (!cancelled) {
			...
			Collections.sort(listDir,
					Comparators.getForDirectory(sortBy, ascending));
			Collections.sort(listFile,
					Comparators.getForFile(sortBy, ascending));
		}
		...
		if (!cancelled) {
			...
			Message msg = handler
					.obtainMessage(MESSAGE_SHOW_DIRECTORY_CONTENTS);
			msg.obj = contents;
			msg.sendToTarget();
		}
...
class Comparators {
	...
	public static Comparator<FileHolder> getForFile(int comparator,
			boolean ascending) {
		switch (comparator) {
		case NAME:
	...
	public static Comparator<FileHolder> getForDirectory(int comparator,
			boolean ascending) {
		switch (comparator) {
		case NAME:
			return new NameComparator(ascending);
		case SIZE:
			return new NameComparator(ascending); // Not a bug! Getting
													// directory's size is very
													// slow
		case EXTENSION:
			return new NameComparator(ascending); // Sorting by name as folders
...
class LastModifiedComparator extends FileHolderComparator {
	...
	@Override
	protected int comp(FileHolder f1, FileHolder f2) {
		return ((Long) f1.getFile().lastModified()).compareTo(f2.getFile()
				.lastModified());
	}
}