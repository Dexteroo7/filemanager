...
public class PickFileListFragment extends SimpleFileListFragment {
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		ViewFlipper modeSelector = (ViewFlipper) view
				.findViewById(R.id.modeSelector);

		// Folder init
		if (getArguments()
				.getBoolean(FileManagerIntents.EXTRA_DIRECTORIES_ONLY)) {
			...
			if (getArguments()
					.containsKey(FileManagerIntents.EXTRA_BUTTON_TEXT)) {
				button.setText(getArguments().getString(
		...
		// Files init
			...
			mPickBar.setButtonText(getArguments().getString(
					FileManagerIntents.EXTRA_BUTTON_TEXT));
			...
			mPickBar.setOnPickRequestedListener(new OnPickRequestedListener() {
				...
				public void pickRequested(String filename) {
					if (filename.trim().length() == 0) {
						Toast.makeText(getActivity(), R.string.choose_filename,
	 ...
	 * 
	 * @param selection
	 *            A {@link File} representing the user's selection.
	 * @param getContentInitiated
	 *            Whether the fragment was called through a GET_CONTENT intent
	 *            on the IntentFilterActivity. We have to know this so that
	 *            result is correctly formatted.
	 */
	private void pickFileOrFolder(File selection, boolean getContentInitiated) {
		Intent intent = new Intent();
		intent.putExtras(getArguments());

		PreferenceActivity.setDefaultPickFilePath(getActivity(),
				selection.getParent() != null ? selection.getParent() : "/");
