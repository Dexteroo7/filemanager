...
public class BookmarkListFragment extends ListFragment {
	...
	public void onViewCreated(View view, Bundle savedInstanceState) {
		...
		getListView().setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScrollStateChanged(AbsListView view, int scrollState) {
				if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
					((BookmarkListAdapter) getListAdapter())
							.setScrolling(false);
				} else
		...
		if (android.os.Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
			...
			BookmarkMultiChoiceModeHelper.listView_setMultiChoiceModeListener(
					getListView(), getActivity());
	...
	public void onListItemClick(ListView l, View v, int position, long id) {
		String path = ((BookmarkListAdapter.Bookmark) getListAdapter().getItem(
				position)).path;
