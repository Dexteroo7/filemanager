...
public abstract class MenuUtils {
	 ...
	 * @param menuResource
	 *            The resource id of the menu to load.
	 * @param mi
	 ...
	 */
	static public void fillMultiselectionMenu(Menu m, int menuResource,
			MenuInflater mi) {
	 ...
	 * for the passed {@link FileHolder} object.
	 * 
	 * @param m
	 *            The {@link Menu} to fill.
	 * @param menuResource
	...
	/**
	 * Central point where we handle actions for multiple selection, for every
	 * API level.
	 * 
	 * @param mItem
	 *            The selected menu option/action.
	 * @param fItems
	 *            The data to act upon.
	 */
	public static boolean handleMultipleSelectionAction(
			final FileListFragment navigator, MenuItem mItem,
			List<FileHolder> fItems, Context context) {
		...
		case R.id.menu_move:
			((FileManagerApplication) navigator.getActivity().getApplication())
					.getCopyHelper().cut(fItems);

			// Refresh options menu
			if (VERSION.SDK_INT >= VERSION_CODES.HONEYCOMB)
				ActionbarRefreshHelper.activity_invalidateOptionsMenu(navigator
						.getActivity());
			break;
		case R.id.menu_copy:
			((FileManagerApplication) navigator.getActivity().getApplication())
					.getCopyHelper().copy(fItems);
	...
	/**
	 * Central point where we handle actions for single selection, for every API
	 * level.
	 * 
	 * @param mItem
	 *            The selected menu option/action.
	...
	public static boolean handleSingleSelectionAction(
			final SimpleFileListFragment navigator, MenuItem mItem,
			FileHolder fItem, Context context) {
		DialogFragment dialog;
		Bundle args;
		...
		case R.id.menu_compress:
			...
			dialog.setArguments(args);
			dialog.show(navigator.getFragmentManager(),
					SingleCompressDialog.class.getName());
			return true;

		case R.id.menu_extract:
			File dest = new File(fItem.getFile().getParentFile(),
			...
			// way.
			new ExtractManager(context).setOnExtractFinishedListener(
