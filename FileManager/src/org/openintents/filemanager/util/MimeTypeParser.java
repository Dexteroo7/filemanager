...
public class MimeTypeParser {
	...
	public MimeTypeParser(Context ctx, String packagename)
			throws NameNotFoundException {
		this.packagename = packagename;
		resources = ctx.getPackageManager().getResourcesForApplication(
				packagename);
	}
	...
	private void addMimeTypeStart() {
		...
		if (icon != null) {
			int id = resources.getIdentifier(
					icon.substring(1) /* to cut the @ */, null, packagename);
			if (id > 0) {
